...
La teoría de la programación se preocupa primordialmente con modelos matemáticos que se relacionan al proceso de programación. El desarrollo de modelos útiles, los cuales derivan en técnicas de solución y perspectivas prácticas, han sido la interfaz continua entre la teoría y la práctica. 
%
...
\textbf{Tiempo de procesamiento} ($p_{ij}$) La $p_{ij}$ representa el tiempo de procesamiento de un trabajo $j$ en una máquina $i$. El subíndice $i$ es omitido si el tiempo de procesamiento del trabajo $j$ no depende de la máquina o si el trabajo $j$ únicamente va a ser procesado en una máquina dada \citep[p.~14]{Pinedo1995}.

\textbf{Fecha de liberación} ($r_j$) La fecha de liberación $r_j$ de un trabajo $j$ también es conocido como fecha de inicio \citetext{p.~14}.

\textbf{Fecha de cierre} ($d_j$) La fecha de cierre $d_j$ del trabajo $j$ representa la fecha comprometida de compleción. La compleción de un trabajo después de su fecha de cierre \textit{es} permitida, pero se incurre en una penalización. Cuando una fecha de cierre \textit{debe} ser cumplida se le llama \textit{fecha límite} y es denotada por $\bar{d}_j$ \citetext{p.~14}.
...
Al hacer frente a un problema de programación, se le debe identificar antes de buscar su solución. Aceptando que el problema es complicado y para saber si ya ha sido resuelto en la literatura, se hace uso de una notación reconocida \citep[p.~7]{TKindt2002}. Un problema de programación es descrito por una tripleta $\alpha\: |\: \beta\: |\: \gamma$. El campo $\alpha$ describe el entorno de la máquina y contiene únicamente un valor. El campo $\beta$ provee detalles de las características del procesamiento y restricciones, puede no tener valor alguno, un solo valor o múltiples valores. El campo $\gamma$ describe el objetivo a ser minimizado y frecuentemente contiene un solo valor \citep[p.~14]{Pinedo1995}.

Los posibles entornos de máquinas especificadas en el campo $\alpha$ son:

\textbf{Máquina individual} ($1$) El caso de una máquina individual es el más simple de todos los entornos de máquina posibles y es un caso especial de todos los demás entornos de máquina más complicados. Concierne a un taller básico o uno en donde una máquina individual presenta un problema de programación real. Una área de aplicación directa se encuentra en la computación, si pensamos que la máquina individual es el procesador de un ordenador. Los trabajos a ser procesados son necesariamente de una operación \citep[p.~8, p.~14]{TKindt2002,Pinedo1995}.

\textbf{Máquinas idénticas en paralelo} ($Pm$) Hay una cantidad idéntica de máquinas en paralelo. El trabajo $j$ requiere una operación individual y puede ser procesado por cualquiera de las máquinas $m$ o por cualquiera que pertenezca al subconjunto dado. Si el trabajo $j$ no puede ser procesado en cualquier máquina, sino únicamente en alguna perteneciente a un subconjunto $M_j$, entonces la entrada $M_j$ aparece en el campo $\beta$ \citep[p.~14]{Pinedo1995}.

\textbf{Máquinas en paralelo con velocidades distintas} ($Qm$) Hay múltiples máquinas $m$ en paralelo de distintas velocidades. La velocidad de la máquina $i$ es denotada por $v_i$. El tiempo $p_{ij}$ que el trabajo $j$ utiliza en la máquina $i$ es igual a $p_j/v_j$ –asumiendo que el trabajo $j$ recibe todo su procesamiento de la máquina $i$–. Este entorno es referido como máquinas uniformes. Si todas las máquinas tienen la misma velocidad, esto es $v_i = 1$ para toda $i$ y $p_{ij} = p_j$, entonces el entorno es idéntico al anterior \citetext{p.~14}.
