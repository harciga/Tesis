...
\begin{itemize}
  ...
  \item $E_{\max} = \max(E_i)$ siendo $E_i = \max(0; d_i - C_i)$: la máxima anticipación (\textit{earliness}) \citep[p.~13]{TKindt2002}.
\end{itemize}

Generalmente, $\fint_{\max}$ se refiere al criterio ordinario «minimax», el cual es una función no\nobreakdash-decreciente de los tiempos de compleción de los trabajos en $C_1,\dots,C_n$, estas funciones objetivo son llamadas medidas de desempeño regulares. Este no es el caso para el criterio $E_{\max}$ \citep[p.~13]{TKindt2002}. Recientemente los investigadores han empezados a estudiar funciones objetivo que no son regulares. Por ejemplo, cuando el trabajo $j$ tiene la fecha de cierre $d_j$, puede estar sujeto a una penalización por anticipación, en donde la «anticipación» del trabajo $j$ se define como 
\[ E_j = \max (d_j - C_j, 0) \]
...
El peso asociado a la anticipación del trabajo $j$ ($w'_j$) puede ser diferente al peso asociado a la tardanza del trabajo $j$ ($w''_j$) \citep[p.~19]{Pinedo1995}.
...
\begin{itemize}
  ...
  \item $\bar{E}^w$ es el promedio ponderado de anticipación de los trabajos \citep[p.~14]{TKindt2002}.
\end{itemize}
