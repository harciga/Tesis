...
Cuando todo el procesamiento del trabajo $j$ es llevado a cabo en la misma máquina $k$, el tiempo $p_{ijk}$ que las operaciones $i$ del trabajo $j$ utilizan de la máquina $k$ es igual a $p_{ij}/v_k$. Este entorno es referido como máquinas uniformes. Si todas las máquinas tienen la misma velocidad, esto es $v_k = 1$ para toda $jk$ y $p_{ijk} = p_{ij}$, entonces el entorno es idéntico al anterior \citetext[p.~14, p.~8]{Pinedo1995,TKindt2002}.
