...
Si bien el gestor escolar en el sector público no cuenta con el mismo grado de laxitud operativa que su contra-parte\marginnote{revisar el uso del gui\'on} en la iniciativa privada, ambos tienen alternativas de acción similares para afectar el desempeño de su institución.
...
Este capítulo es una revisión bibliográfica enfocada en los aspectos fundamentales del área de investigación. Introduce la definición del problema general de planificación\marginnote{planificaci\'on?} de horarios\index{horarios!planificaci\'on!problema general} así como los términos relevantes al ámbito de estudio \citep[p.~8]{abdullah06heuristic-approaches}.  
...
En el proceso\index{proceso} de programación\index{programación}, es necesario conocer el tipo y cantidad de cada recurso\index{recurso} de manera que se pueda determinar cuándo las tareas\index{tarea} pueden ser factiblemente\index{factible} realizadas. Cuando se especifícan los recursos\index{recurso}, se define efectivamente el alcance del problema de programación\index{programación!problema general}. Además, se describe cada tarea\index{tarea} en términos informativos tales como su requisito de recurso\index{recurso}, su duración, el tiempo\index{tiempo} más pronto en el que puede comenzar, y el tiempo\index{tiempo} en el que debe llegar a su compleción.
En general, la duración de la tarea\index{tarea} es incierta, pero se puede suprimir la incertidumbre –si así se desea– cuando se formula el problema\index{problema}. También se debe describir cualquier limitante\index{limitante} procedimental\index{limitante!procedimental} –restricciones de precedencia– que exista entre las tareas\index{tarea}. La información acerca de los recursos\index{recurso} y las tareas\index{tarea} define un problema de programación\index{programación!problema general}. Sin embargo, encontrar una solución es comúnmente una tarea compleja, y los planteamientos formales\index{planteamiento formal} para la solución del problema son beneficiosos.
\iffalse
