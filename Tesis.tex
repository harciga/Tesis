...
En todos los problemas de programación se considera que el número de trabajos y el número de máquinas son finitos. El número de trabajos es denominado por $n$ y el número de máquinas por $m$. Usualmente el subíndice $j$ se refiere a un trabajo mientras que el subíndice $i$ se refiere a una máquina. Si un trabajo requiere de varios pasos u operaciones, entonces el par $(i, j)$ se refiere al paso del proceso u operación del trabajo $j$ en la máquina $i$. Los siguientes atributos están asociados a un trabajo $j$.

\textbf{Tiempo de procesamiento} ($p_{ij}$) La $p_{ij}$ representa el tiempo de procesamiento de un trabajo $j$ en una máquina $i$. El subíndice $i$ es omitido si el tiempo de procesamiento del trabajo $j$ no depende de la máquina o si el trabajo $j$ únicamente va a ser procesado en una máquina dada.

\textbf{Fecha de liberación} ($r_j$) La fecha de liberación $r_j$ de un trabajo $j$ también es conocido como fecha de inicio. 

\textbf{Fecha de cierre} ($d_j$) La fecha de cierre $d_j$ del trabajo $j$ representa la fecha comprometida de compleción. La compleción de un trabajo después de su fecha de cierre \textit{es} permitida, pero se incurre en una penalización. Cuando una fecha de cierre \textit{debe} ser cumplida se le llama \textit{fecha límite} y es denotada por $\bar{d}_j$.

\textbf{Peso} ($w_j$) El peso $w_j$ del trabajo $j$ es básicamente un factor de prioridad, denotando la importancia del trabajo $j$ en relación a otros trabajos en el sistema. Por ejemplo, este peso puede representar el costo actual de mantener el trabajo en el sistema. Este costo podría ser un activo o un costo de inventario; también podría representar la cantidad de valor ya agregado al trabajo.

Un problema de programación es descrito por una tripleta $\alpha\; |\; \beta\; |\; \gamma$. El elemento $\alpha$ describe el ambiente de la máquina y contiene únicamente un valor. El elemento $\beta$ provee detalles de las características del procesamiento y restricciones, puede no tener valor alguno, un solo valor o múltiples valores. El elemento $\gamma$ describe el objeto a ser minimizado y frecuentemente contiene un solo valor.

Los posibles ambientes para especificar en el elemento $\alpha$ son:

\textbf{Máquina única}

\textbf{Máquinas idénticas en paralelo}

\textbf{Máquinas en paralelo con velocidades distintas}

\textbf{Máquinas no-relacionadas en paralelo}
